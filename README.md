...
Приложение использует систему защищенных маршрутов. Пользователи должны быть авторизованы для доступа к разделам `/crm` и `/orders`. Неавторизованные пользователи перенаправляются на страницу `/login`.
